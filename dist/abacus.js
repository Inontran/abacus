...
var View = /** @class */ (function () {
    ...
    View.prototype._createViewHandles = function (abacusProperty) {
        ...
        for (var i = 0; i < handleIndexes.length; i += 1) {
            ...
            viewInstance._handles[handleIndex].htmlElement.addEventListener('mousedown', viewInstance._handlerHandleItemClickStart);
            viewInstance._handles[handleIndex].htmlElement.addEventListener('touchstart', viewInstance._handlerHandleItemClickStart, { passive: true });
        }
    };
    /**
    ...
    View.prototype._bindEventListeners = function () {
        ...
        document.addEventListener('mousemove', viewInstance._handlerHandleItemClickMove, { passive: true });
        document.addEventListener('touchmove', viewInstance._handlerHandleItemClickMove, { passive: true });
        viewInstance._handlerHandleItemClickStop = viewInstance._handlerHandleItemClickStop.bind(viewInstance);
        document.addEventListener('mouseup', viewInstance._handlerHandleItemClickStop);
        document.addEventListener('touchend', viewInstance._handlerHandleItemClickStop);
        document.addEventListener('touchcancel', viewInstance._handlerHandleItemClickStop);
    };
    /**
     * Обработчик обновления модели.
     * @private
    ...
    View.prototype._handlerHandleItemClickStart = function (event) {
        ...
        var handleHtml = event.currentTarget;
        var handleIndexAttr = handleHtml.getAttribute('data-handle-index');
        if (!handleIndexAttr) {
            return;
        }
        var handleIndex = parseInt(handleIndexAttr, 10);
    ...
    View.prototype._bindEventListenersOnMarks = function () {
        ...
        this._collectionMarks.forEach(function (mapItem) {
            var mark = mapItem;
            mark.htmlElement.addEventListener('click', _this._handlerClickMarks);
            mark.htmlElement.addEventListener('touchend', _this._handlerClickMarks);
        });
    };
    /**
     ...
     * @param {Event} event Объект события.
     */
    View.prototype._handlerClickMarks = function (event) {
        var _a;
        event.preventDefault();
        var viewInstance = this;
        if (viewInstance._isDisabled) {
        ...
        var markHtml = event.currentTarget;
        var markValueAttr = markHtml.getAttribute('data-associated-value');
        if (!markValueAttr) {
            return;
        }
        var markAssociatedValue = parseFloat(markValueAttr);
        if (((_a = viewInstance._cachedAbacusProperty) === null || _a === void 0 ? void 0 : _a.value) !== markAssociatedValue) {
...
/******/ })()
;
//# sourceMappingURL=abacus.js.map?v=897ed2919b624e66c5ab